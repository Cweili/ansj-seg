...
public class UserDefineLibrary {
	...
	private static final Log log = LogFactory.getLog(UserDefineLibrary.class);
	...
	static {
		try {
			...
			log.info("init user library ok use time :" + (System.currentTimeMillis() - start));
		} catch (Exception e) {
	...
	public static void loadFile(Forest forest, File file) {
		// TODO Auto-generated method stub
		if (!file.canRead()) {
			log.error("file in path " + file.getAbsolutePath() + " can not to read!");
			return;
		}
		String temp = null;
		BufferedReader br = null;
	...
	public static void loadLibrary(Forest forest, String temp) {
		...
		if (temp == null) {
			try {
				temp = MyStaticValue.rb.getString("userLibrary");
			} catch (Exception e) {
		...
		if (temp != null) {
			...
			if (file.isFile()) {
				...
				log.error("init user library  error :" + temp + " because : not find that file !");
			}
		}
	}
